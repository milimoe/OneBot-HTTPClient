...
namespace Milimoe.OneBot.Framework.Utility
{
    ...
    /// <para/>文件会保存为：程序目录/configs/<see cref="PluginName"/>/<see cref="FileName"/>.conf
    /// </summary>
    public class PluginConfig : Dictionary<string, object>
    {
        ...
        /// <summary>
        /// 新建一个配置文件，文件会保存为：程序目录/configs/<see cref="PluginName"/>/<see cref="FileName"/>.conf
        /// </summary>
        /// <param name="plugin_name"></param>
        /// <param name="file_name"></param>
        public PluginConfig(string plugin_name, string file_name)
        {
        ...
        public void Parse<T>(string key)
        {
            ...
            {
                T instance = JsonSerializer.Deserialize<T>(value.ToString());
                base[key] = instance;
            }
        }
        ...
        {
            string json = JsonSerializer.Serialize((Dictionary<string, object>)this, new JsonSerializerOptions()
            {
                WriteIndented = true
            });
            string dpath = $@"{AppDomain.CurrentDomain.BaseDirectory}configs/{PluginName}";
