...
namespace Milimoe.OneBot.Framework.JsonConverter
{
    public class GroupBanEventConverter : BaseEntityConverter<GroupBanEvent>
    {
        ...
        public override void ReadPropertyName(ref Utf8JsonReader reader, string propertyName, JsonSerializerOptions options, ref GroupBanEvent result, Dictionary<string, object> convertingContext)
        {
            switch (propertyName)
            {
                case "time":
