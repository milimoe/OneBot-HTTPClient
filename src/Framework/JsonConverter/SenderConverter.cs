...
namespace Milimoe.OneBot.Framework.JsonConverter
{
    public class SenderConverter : BaseEntityConverter<Sender>
    {
        ...
        public override void ReadPropertyName(ref Utf8JsonReader reader, string propertyName, JsonSerializerOptions options, ref Sender result, Dictionary<string, object> convertingContext)
        {
            switch (propertyName)
            {
                case "user_id":
